...
def ap_per_class(tp, conf, pred_cls, target_cls, pr_score):
    ...
    # pr_score = 0.2  # score to evaluate P and R https://github.com/ultralytics/yolov3/issues/898
    ...
    for ci, c in enumerate(unique_classes):
        ...
        if n_p == 0 or n_gt == 0:
            ...
            r_curve = interp1d(-conf[i], recall, axis=0, bounds_error=False, fill_value=(recall[0], recall[-1]))
            r[ci] = r_curve(-pr_score)  # r at pr_score, negative x, xp because xp decreases

            # Precision
            precision = tpc / (tpc + fpc)  # precision curve
            p_curve = interp1d(-conf[i], precision, axis=0, bounds_error=False, fill_value=(precision[0], precision[-1]))
            p[ci] = p_curve(-pr_score)  # p at pr_score

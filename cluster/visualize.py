...
def getclusters(filename, edge=0):   # get cluster arrays from a file
    ...
    xmin, ymin, xmax, ymax, size, xall, yall = [], [], [], [], [], [], []
    for i in range(len(data)):
        tmp = data[i].strip().split()
        size.append(eval(tmp[0]))
        xmin.append(eval(tmp[1]) - edge)
        ymin.append(eval(tmp[2]) - edge)
        xmax.append(eval(tmp[3]) + edge)
        ymax.append(eval(tmp[4]) + edge)
        xx = []
        yy = []
        for s in range(size[i]):
            xx.append(eval(tmp[2*s+5]))
            yy.append(eval(tmp[2*s+6]))
        xall.append(xx)
        yall.append(yy)
    
    return xmin, ymin, xmax, ymax, size, xall, yall
    ...
    with open(filename, 'r') as f:
        ...
        for i in aux:
            if i not in choice:
                continue
            print(data[i])
            x, y = eval(data[i].split()[0]), eval(data[i].split()[1])
...
def annotate_single(year):  # random pick 50 original (608x608) images and annotate predictions
    num = 50
    txtpath = './raw/{}/'.format(year)
    jpgpath = '/mnt/satellite/JPEGImages_{}/'.format(year)
    savepath = './result/{}_anno/'.format(year)
...
def mapcut_cluster(year):
    edge = 100
    edge /= 111000
    filename = './result/{}.txt'.format(year)
    save_path = './result/{}_cluster/'.format(year)
    ...
    for i in range(len(size)):
        ...
        img = getmap((xmin[i],ymin[i],xmax[i],ymax[i]), year)
        # for j in range(len(xall[i])):
        #     xloc = xall[i][j]
        #     yloc = yall[i][j]
        #     xloc_pix = int((xloc - xmin[i]) / (xmax[i] - xmin[i]) * img.shape[1])
        #     yloc_pix = int((ymax[i] - yloc) / (ymax[i] - ymin[i]) * img.shape[0])
        #     img = cv2.circle(img, (xloc_pix, yloc_pix) , 20, (0,0,255), 2)
            
        # xx0, yy0, xx1, yy1 = round(xmin[i], 3), round(ymin[i], 3), round(xmax[i], 3), round(ymax[i], 3)
        # img = cv2.putText(img, 'x:{}~{}, y:{}~{}'.format(xx0, xx1, yy0, yy1), 
        #                   (0, img.shape[0]-8), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 1)
        cv2.imwrite(save_path + '{}.jpg'.format(i), img)
        fout = open(save_path + '{}.txt'.format(i), 'w')
        fout.write('region:\n{} {} {} {}\n'.format(xmin[i], ymin[i], xmax[i], ymax[i]))
        fout.write('targets:\n')
        for j in range(len(xall[i])):
        ...
        targets = []
        for j in range(len(xall[i])):
            targets.append([np.float32(xall[i][j]), np.float32(yall[i][j])])
...
def comparison(years):  # get mapcut for clusters and compare between years
    years = (2010, 2011, 2012, 2013, 2014, 2015, 2016, 2018)
    x_min, x_max, y_min, y_max = (95., 97., 16., 18.) 
    thres = 5
    hull_all = []
    for year in years:
        filename = './result/{}.txt'.format(year)
        xmin, ymin, xmax, ymax, size, xall, yall = getclusters(filename, 0)
        for i in range(len(size)):
            if size[i] < thres:
                continue
            targets = []
...
def heatmap_single(year):  # generate heatmap of targets in whole region, using prepared map
    x_pix = 10000
    x_min, x_max, y_min, y_max = (95., 97., 16., 18.) # if year != 2018 else (1.06e7, 1.08e7, 1.82e6, 2.02e6)
    res = (x_max - x_min) / x_pix
    thres = 5
    bg = cv2.imread('utils/edge_{}/{}.jpg'.format(x_pix, year))
    fin = open('./data/{}.txt'.format(year), 'r')
    ...
    cv2.imwrite('{}.jpg'.format(year), bg)
    
        
def heatmap_cluster(year): 
    ...
    thres = 5
    bg = cv2.imread('utils/edge_{}/{}.jpg'.format(x_pix, year))
    # bg = cv2.imread('utils/map_{}/{}.jpg'.format(x_pix, year))

    xmin, ymin, xmax, ymax, size, xall, yall = getclusters(filename, edge)
    fout = open(save_path + '{}.txt'.format(year), 'w')
    cnt = 0
    for i in range(len(size)):
        if size[i] < thres:
            continue
    ...
    cv2.imwrite(save_path + '{}.jpg'.format(year), bg)
  
    
def heatmap_region(year, single=True, cluster=True):  # get heatmap in specific region
    x_pix = 10000
    channel = 3
    ...
    if single:
        ...
        cv2.imwrite('single_{}_{}_{}_{}_{}.jpg'.format(year, x_min, y_min, x_max, y_max), bg)
    ...
    if cluster:
        ...
        cv2.imwrite('cluster_{}_{}_{}_{}_{}.jpg'.format(year, x_min, y_min, x_max, y_max), bg)

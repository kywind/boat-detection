...
def train(hyp, opt, device, tb_writer=None):
    ...
    # Print hyperparameter data
    print(f'Hyperparameters {hyp}')

    # Initiate logging settings
    log_dir = Path(tb_writer.log_dir) if tb_writer else Path(opt.logdir) / 'evolve'  # log directory
    wdir = str(log_dir / 'weights') + os.sep  # weight directory
    ...
    # Save hyperparameters and opts to logging directory
    ...
    # Initiate model
    # the model architecture in opt.cfg should match the architecture of the model in opt.weights
    ...
    # Define optimizer
    if opt.adam:
        optimizer = optim.Adam(pg0, lr=hyp['lr0'], betas=(hyp['momentum'], 0.999))  # replace beta1 with hyp.momentum
    else:
        optimizer = optim.SGD(pg0, lr=hyp['lr0'], momentum=hyp['momentum'], nesterov=True)
    optimizer.add_param_group({'params': pg1, 'weight_decay': hyp['weight_decay']})  # add weight decay
    ...
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, gs, opt, hyp=hyp, augment=True,
                                            cache=opt.cache_images, rect=opt.rect, local_rank=rank, world_size=opt.world_size)
    ...
    # Model parameters
    hyp['cls'] *= nc / 80.  # scale coco-tuned hyp['cls'] to current dataset; hyp['cls'] is the cls loss gain
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                #     ('WARNING: non-finite loss, ending training ', loss_items)
                #     return results
        ...
        if rank in [-1, 0]:
            ...
            if save:
                ...
                if (epoch+1) % 50 == 0:
                    torch.save(ckpt, last.replace('last.pt','{:03d}.pt'.format(epoch)))
